...
-- inner join sem o on multiplica a tabela, então para n acontecer isso coloca "on"
select
...
-- Para acrescentar o "nome" colocado "c" de cliente e é feito o inner join clientes c, on p.client_id = c.client_id
drop view vw_pedidos;
create view vw_pedidos as
select
...
select c.nome from clientes c where c.cliente_id not in (select client_id from pedidos);